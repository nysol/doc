...
# 文字が全角なら2、半角なら1を返す
def zenhan(char)
    return 2 if char.length != char.bytesize
    return 1 if char.length == char.bytesize
end
...
def exec(scp)
	...
	while true
		...
		#
		logLine=logLine.chomp.sub(/; 2.+$/,'') # 日付時刻は消す
		#logFixLen=logLine.scan(/.{1,90}/) # texの行溢れを防ぐために90文字で分割
		
		line = lenw(logLine,92)
		log << "#{line.chomp.sub(/;.+$/,'')}\n"
	end
	}
	}
