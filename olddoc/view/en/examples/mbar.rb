...
##############  Example 2
title="Draw a one dimensional bar graph matrix"
comment=<<'EOF'
Using the data \verb|dat2.csv|, set \verb|Age| as the unit of composition, draw the bar graph showing the population by age. Specify \verb|Pref| at \verb|k=| parameter, which designates pref on the x-axis, extended horizontally as a one-dimensional bar graph. Specify the title of the graph at \verb|title=| parameter.
EOF
scp=<<'EOF'
more dat2.csv
...
##############  Example 3
title="Draw a two dimensional bar graph matrix"
comment=<<'EOF'
Using the data \verb|dat3.csv|, set \verb|ThemePark| as the unit of composition, draw the bar graph showing the theme park by \verb|number| , and set the \verb|width=| at 200, and \verb|height=| at 150. Specify \verb|Gender| and \verb|Age| at \verb|k=| parameter, which designates Gender on the x-axis extended horizontally, and Age on y-axis extended vertically.
EOF
scp=<<'EOF'
more dat3.csv
