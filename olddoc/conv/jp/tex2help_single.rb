...
# 引数チェック
if ARGV.size != 2 then
	STDERR.puts "mcmd/jpのtexマニュアルをヘルプ用textに変換する単体用"
	STDERR.puts "#{$0} 入力ファイル名"
	STDERR.puts "#{$0} mbonsai.tex xxhelp"
	exit
end
...
def evalUse(line)
	use=nil
	if line=~/^\\subsection/
		if line=~/書式/ or line=~/パラメータ/ or line=~/利用例/  or line=~/正規表現の使用方法/ then 
			use=true
		else
			use=false
		end
...
files.each{|iFile|
	name = File.basename(iFile)
	oFile="./xxsrc/#{name}"
	next if name=="mcmd.tex"
...
Dir["xxsrc/*.h"].each{|iFile|
	...
	# 入力系コマンドの例はtextColorを使っているので、色属性は全て外して文字列のみに変換
	# \\textColor{blue}{red}{abcd} => abcd
	doc.gsub!(/\\textColor{.*?}{.*?}{(.*?)}/,'\1')
