...
**パラメータ**
  ...
  skeys=項目名リスト: 事前にソーティングを行う。そのソーティングキーを指定する。
      例) skyes="amount%nr,customer" # ``amount`` 項目数値逆順+ ``customer`` 項目昇順
...
出力される ``amount`` 項目は文字列( ``"str"`` )となる。
  ...
  .. code-block:: python
    ...
    f=nm.mcut(f="customer,date,amount",i=dat).getline(dtype=dtype,otype="dic",skeys="amount%nr"):
    for line in f:
      print(line)
