...
if len(argv)!=2 and len(argv)!=3:
	print("dbのcmd,smpからrst形式の文書を構成する")
	print("%s mcut"%argv[0])
	print("%s mcut ../../methods0"%argv[0])
	exit()
...
# dbのpath
cmdFile="db.cmd.%s"%name
smpFile="db.smp.%s"%name

oPath=".."
if len(argv)==3:
	oPath=argv[2]
oFile="%s/%s.rst"%(oPath,name)
...
with open(oFile,"w") as fpw:
	...
	if cmd.db["comParams"]!="":
		...
		para=""
		i=0
		for param in cmd.db["comParams"].split(","):
			if i==0:
				para+=":ref:`%s=<common_param_%s>`\n"%(param,param)
			else:
				para+=", :ref:`%s=<common_param_%s>`\n"%(param,param)
			i+=1
		fpw.write(para)
